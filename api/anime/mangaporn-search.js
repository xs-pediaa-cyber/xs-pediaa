...
router.get('/', async (req, res) => {
  try {
    ...
    if (!q) {
      return res.status(400).json({ success: false, error: "Missing 'q' or 'query' parameter" });
    }
    const page = parseInt(req.query.page) || 1;
    const limit = parseInt(req.query.limit) || 20;

    const html = await fetchHTML(`${BASE}/search?q=${encodeURIComponent(q)}&page=${page}`);
