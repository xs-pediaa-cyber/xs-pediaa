/**
 * NAMA SCRAPE  :: ANIMEINDO GENRE FILTER
...
router.get('/', async (req, res) => {
  try {
    ...
    if (!slug) {
      return res.status(400).json({ status: false, error: "Parameter 'slug' wajib diisi. Contoh: ?slug=action&page=1" });
    }
    if (!pageParam) {
      return res.status(400).json({ status: false, error: "Parameter 'page' wajib diisi." });
    }
    ...
    return res.json({ status: true, data: { slug: cleanSlug, page, items } });
  } catch (err) {
